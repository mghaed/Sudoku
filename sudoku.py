...
def solve(bo):
    find = find_empty(bo) #return the possition of row and col
    if find: #we find the posision of empty
        row, col = find
    else:
        return True
    
    for i in range(1,10):
       if valid(bo, i, (row, col)): # if the i that we enterde is valid
           bo[row][col] = i #we add it into the board
           
           if solve(bo): #recursively calling the solution
               return True
           else:
               bo[row][col] = 0 # reset the last element thet we just added
    return False #if the i that we added is not correct
